...
class drwnPixelMarginal	//: public drwnStdObjIface
{
public:
	//! Domain type
...
public:
    ...
    //! Copy constructor
    drwnPixelMarginal(const drwnPixelMarginal& m);
	...
	//! Return type
    virtual const char *type() const { return "drwnPixelMarginal"; }

    //! Make copy of Bayesian network from existing one
    virtual drwnPixelMarginal *clone() const { return new drwnPixelMarginal(*this); }

    //! number of bytes comsumed by the factor data (excludes variables lists, etc)
    size_t memory() const { return sizeof(drwnPixelMarginal); }
...
//! Function to learn parameters by MLE from sample
template <class G>
void drwnPixelMarginal::learn(unsigned int varIndx, drwnSamples<G>& samples)
{
